...
import 'package:new_sali_backend/src/shared/dependencies/shelf_cors_headers_base/shelf_cors_headers_base.dart';
...
Response responseError(String message,
  ...
  return Response(statusCode, body: errorMsg, headers: defaultHeaders);
}
...
void main(List<String> args) async {
  ...
  for (var i = 0; i < numberOfIsolates - 1; i++) {
    await spawnIsolate(i, arguments);  
  }
  await spawnIsolate(numberOfIsolates - 1, arguments);
}

Future<BidirectionalStreamIsolate> spawnIsolate(
    int id, List<dynamic> arguments) async {
  final streamIsolate = await StreamIsolate.spawnBidirectional(isolateMain,
      debugName: id.toString(), argument: [id, ...arguments]);
...
void routes(Router app, CollectorRegistry reg) {
  ...
  app.get('$basePath/protocolo/processos/public/site/<ano>/<codigo>',
    ...
    try {
      ...
      return Response.ok(
        jsonEncode({}, toEncodable: SaliCoreUtils.customJsonEncode),
...
String _message(DateTime requestTime, int statusCode, Uri requestedUri,
  ...
  return
      // '${requestTime.toIso8601String().substring(0, 19)} '
      '${BackendUtils.prettyDuration2(elapsedTime).padRight(16)} '
          '${method.padRight(7)} [$statusCode] ' // 7 - longest standard HTTP method
          '${requestedUri.path}${_formatQuery(requestedUri.query)}';
  //'  i: ${Isolate.current.debugName}';
}
