...
import 'package:new_sali_frontend/src/shared/directives/dropdown_menu_directive.dart';
import 'package:new_sali_frontend/src/shared/directives/safe_append_html_directive.dart';
import 'package:new_sali_frontend/src/shared/directives/safe_inner_html_directive.dart';
    ...
    if (gridItems.isNotEmpty &&
      ...
      grids.forEach((grid) {
        ...
        if (grid['ncol'] != ncol) {
          ...
          (grid['items'] as List)
              .forEach((c) => c.style.removeProperty('margin-top'));
          ...
          if ((grid['ncol'] as int) > 1) {
            ...
            for (var i = 0; i < sublist.length; i++) {
              ...
              var prev_fin = items[i].getBoundingClientRect().bottom;
              // top edge of current item
              var curr_ini = c.getBoundingClientRect().top;

              var marginTop =
                  '${prev_fin + (grid['gap'] as double) - curr_ini}px';
...
// ---------------- evento de ordenação ----------------
  ...
  void exportXlsx() {
    ...
    FrontUtils.downloadFile2(bytes, 'teste.xlsx',
    ...
    docPdf.addPage(
      pdf.MultiPage(
        ...
        build: (pdf.Context context) => <pdf.Widget>[
          pdf.Header(level: 1, text: 'Relatório'),
          pdf.Table.fromTextArray(context: context, data: <List<String>>[
            settings.colsDefinitions.map((e) => e.title).toList(),
            ...rows
                .map((row) => row.columns.map((col) => col.value).toList())
                .toList()
          ]),
        ],
        footer: (pdf.Context context) {
          return pdf.Container(
            ...
            decoration: pdf.BoxDecoration(
                border:
                    pdf.Border(top: pdf.BorderSide(color: pdf.PdfColors.grey))),
            child: pdf.Padding(
              padding: pdf.EdgeInsets.only(top: 5),
              child: pdf.Text('Sistema  - 2023',
                  style: pdf.TextStyle(fontSize: 9, color: pdf.PdfColors.grey)),
            ),
          );
        },
      ),
