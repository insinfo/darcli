...
final __DIR__ = Directory.current.path; //dirname(Platform.script.toFilePath());
...
class SendMailGmailApi {
  ...
  String redirectUri =
      'urn:ietf:wg:oauth:2.0:oob'; //	urn:ietf:wg:oauth:2.0:oob para aplicativos instalados 
  String scope = 'https://mail.google.com';
  String userAgent = 'app';
  ...
  String refreshToken =
      "refreshToken";
  //inicializa a class com as configurações de conta   desenv.pmro@gmail.com
...
// UTF-8 characters in names and subject
  ...
  Future<void> refreshOAuthToken() async {
    ...
    //print('SendMailGmailApi@refreshOAuthToken');
    final isTokenExpired = await isAccessTokenExpired();
    if (isTokenExpired == false) {
      return;
    }

    //pega um novo  access_token
  ...
  Future<String> getRefreshToken(String codigoAutorizacao) async {
    ...
    print('SendMailGmailApi@getRefreshToken');
    if (resp.statusCode == 200) {
