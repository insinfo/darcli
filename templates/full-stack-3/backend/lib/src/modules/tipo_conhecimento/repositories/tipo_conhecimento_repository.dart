...
class TipoConhecimentoRepository {
  final Connection db;
  TipoConhecimentoRepository(this.db);

  /// lista todos
  Future<DataFrame<Map<String, dynamic>>> getAll({Filters? filtros}) async {
    final query = db.table(TipoConhecimento.fqtn);
  ...
  Future<Map<String, dynamic>> getByIdAsMap(int id) async {
    final query =
        db.table(TipoConhecimento.fqtn).selectRaw('*').where('id', '=', id);
  ...
  Future<int> create(TipoConhecimento item, {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(TipoConhecimento.fqtn);
    await query.insert(item.toInsertMap());
    return item.id;
  }

  Future<void> update(TipoConhecimento item, {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(TipoConhecimento.fqtn).where('id', '=', item.id);
    await query.update(item.toUpdateMap());
  }

  Future<void> removeById(int id, {Connection? connection}) async {
    final conn = connection ?? db;
    await conn.table(TipoConhecimento.fqtn).where('id', '=', id).delete();
  }

  Future<void> removeAllInTransaction(List<TipoConhecimento> items) async {
