...
class ExperienciaCandidatoCargoRepository {
  final Connection db;
  ExperienciaCandidatoCargoRepository(this.db);

  /// lista
  Future<DataFrame<Map<String, dynamic>>> getAll({Filters? filtros}) async {
    final query = db.table(ExperienciaCandidatoCargo.fqtn);
  ...
  Future<int> create(ExperienciaCandidatoCargo item,
      {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(ExperienciaCandidatoCargo.fqtn);
    await query.insert(item.toInsertMap());
    return item.id;
  }

  Future<void> update(ExperienciaCandidatoCargo item,
      {Connection? connection}) async {
    final conn = connection ?? db;
    final query =
        conn.table(ExperienciaCandidatoCargo.fqtn).where('id', '=', item.id);
  ...
  Future<void> removeAllInTransaction(
      List<ExperienciaCandidatoCargo> items) async {
