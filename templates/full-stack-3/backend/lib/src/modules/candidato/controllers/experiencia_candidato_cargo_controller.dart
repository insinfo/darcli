...
class ExperienciaCandidatoCargoController {
  static Future<dynamic> getAll(RequestContext req, ResponseContext res) async {
    try {
      final repo = req.container!.make<ExperienciaCandidatoCargoRepository>();
      final data = await repo.getAll(filtros: req.filtros);

      return res.responseDataFrame(data);
    } catch (e, s) {
      print('ExperienciaCandidatoCargoController@getAll $e $s');
  ...
  static Future<dynamic> getById(
    ...
    try {
      ...
      print('ExperienciaCandidatoCargoController@getById $e $s');
      return res.responseError(StatusMessage.LIST_ERROR_MESSAGE,
          exception: e, stackTrace: s);
    }
  }
  ...
  static Future create(RequestContext req, ResponseContext res) async {
    try {
      final repo = req.container!.make<ExperienciaCandidatoCargoRepository>();
      await repo
          .create(ExperienciaCandidatoCargo.fromMap(await req.dataAsMap()));
      res.responseSuccess();
    } catch (e, s) {
      print('ExperienciaCandidatoCargoController@create $e $s');
      return res.responseError(StatusMessage.LIST_ERROR_MESSAGE,
          exception: e, stackTrace: s);
    }
  }
  ...
  static Future update(RequestContext req, ResponseContext res) async {
    try {
      final repo = req.container!.make<ExperienciaCandidatoCargoRepository>();
      await repo
          .update(ExperienciaCandidatoCargo.fromMap(await req.dataAsMap()));
      res.responseSuccess();
    } catch (e, s) {
      print('ExperienciaCandidatoCargoController@update $e $s');
      return res.responseError(StatusMessage.LIST_ERROR_MESSAGE,
          exception: e, stackTrace: s);
    }
  }
  ...
  static Future removeAll(RequestContext req, ResponseContext res) async {
    try {
      ...
      final repo = req.container!.make<ExperienciaCandidatoCargoRepository>();
      final items =
          data.map((e) => ExperienciaCandidatoCargo.fromMap(e)).toList();
      await repo.removeAllInTransaction(items);
      return res.responseSuccess();
    } catch (e, s) {
      print('ExperienciaCandidatoCargoController@removeAll $e $s');
