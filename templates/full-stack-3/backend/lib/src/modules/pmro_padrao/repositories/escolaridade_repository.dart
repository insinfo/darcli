...
class EscolaridadeRepository {
  ...
  Future<Map<String, dynamic>?> getByIdAsMap(int id) async {
    final query = db.table(Escolaridade.fqtn).selectRaw('''* ''');
    ...
    if (data == null) {
      //throw Exception('Não encontrado Escolaridade com o id = $id');
      return null;
    }
  ...
  Future<int> create(Escolaridade item, {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(Escolaridade.fqtn);
    await query.insert(item.toInsertMap());
    return item.id;
  }

  Future<void> update(Escolaridade item, {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(Escolaridade.fqtn).where('id', '=', item.id);
    await query.update(item.toUpdateMap());
  }

  Future<void> removeById(int id, {Connection? connection}) async {
    final conn = connection ?? db;
    await conn.table(Escolaridade.fqtn).where('id', '=', id).delete();
  }

  Future<void> removeAllInTransaction(List<Escolaridade> items) async {
