...
class TipoDeficienciaRepository {
  final Connection db;
  TipoDeficienciaRepository(this.db);

  /// lista todos os Tipos de Deficiencia
  Future<DataFrame<Map<String, dynamic>>> getAll({Filters? filtros}) async {
    final query = db.table(TipoDeficiencia.fqtn);
   ...
   Future<Map<String, dynamic>?> getByIdAsMap(int id) async {
    final query = db.table(TipoDeficiencia.fqtn).selectRaw('*').where('id', '=', id);
    final data = await query.first();
    //if (data == null) {
      //throw Exception('Não encontrado Tipo de Deficiencia com o id = $id');
    //}
    return data;
  }

  Future<int> create(TipoDeficiencia item, {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(TipoDeficiencia.fqtn);
    await query.insert(item.toInsertMap());
    return item.id;
  }

  Future<void> update(TipoDeficiencia item, {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(TipoDeficiencia.fqtn).where('id', '=', item.id);
    await query.update(item.toUpdateMap());
  }

  Future<void> removeById(int id, {Connection? connection}) async {
    final conn = connection ?? db;
    await conn.table(TipoDeficiencia.fqtn).where('id', '=', id).delete();
  }

  Future<void> removeAllInTransaction(List<TipoDeficiencia> items) async {
