...
class TelefoneRepository {
  ...
  Future<int> create(Telefone item, {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(Telefone.fqtn);
    await query.insert(item.toInsertMap());
    return item.id;
  }

  Future<void> update(Telefone item, {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(Telefone.fqtn).where('id', '=', item.id);
    await query.update(item.toUpdateMap());
  }

  Future<void> removeById(int id, {Connection? connection}) async {
    final conn = connection ?? db;
    await conn.table(Telefone.fqtn).where('id', '=', id).delete();
  }

  Future<void> removeAllInTransaction(List<Telefone> items) async {
