...
class AuditoriaController {
  static Future<dynamic> getAll(RequestContext req, ResponseContext res) async {
    try {
      ...
      print('AuditoriaController@getAll $e $s');
  ...
  static Future<dynamic> getById(
    ...
    try {
      ...
      print('AuditoriaController@getById $e $s');
      return res.responseError(StatusMessage.LIST_ERROR_MESSAGE,
          exception: e, stackTrace: s);
    }
  }
  ...
  static Future create(RequestContext req, ResponseContext res) async {
    try {
      final repo = req.container!.make<AuditoriaRepository>();
      await repo.create(Auditoria.fromMap(await req.dataAsMap()));
      res.responseSuccess();
    } catch (e, s) {
  ...
  static Future update(RequestContext req, ResponseContext res) async {
    try {
      final repo = req.container!.make<AuditoriaRepository>();
      await repo.update(Auditoria.fromMap(await req.dataAsMap()));
      res.responseSuccess();
    } catch (e, s) {
      print('AuditoriaController@update $e $s');
      return res.responseError(StatusMessage.LIST_ERROR_MESSAGE,
          exception: e, stackTrace: s);
    }
  }
  ...
  static Future removeAll(RequestContext req, ResponseContext res) async {
    try {
      ...
      // }
      final repo = req.container!.make<AuditoriaRepository>();
      final items = data.map((e) => Auditoria.fromMap(e)).toList();
      await repo.removeAllInTransaction(items);
      return res.responseSuccess();
    } catch (e, s) {
      print('AuditoriaController@removeAll $e $s');
