...
class FaixaTempoResidenciaRepository {
  final Connection db;
  FaixaTempoResidenciaRepository(this.db);

  /// lista todos 
  Future<DataFrame<Map<String, dynamic>>> getAll({Filters? filtros}) async {
    final query = db.table(FaixaTempoResidencia.fqtn);
  ...
  Future<Map<String, dynamic>> getByIdAsMap(int id) async {
    final query =
        db.table(FaixaTempoResidencia.fqtn).selectRaw('*').where('id', '=', id);
  ...
  Future<int> create(FaixaTempoResidencia item, {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(FaixaTempoResidencia.fqtn);
    await query.insert(item.toInsertMap());
    return item.id;
  }

  Future<void> update(FaixaTempoResidencia item, {Connection? connection}) async {
    final conn = connection ?? db;
    final query = conn.table(FaixaTempoResidencia.fqtn).where('id', '=', item.id);
    await query.update(item.toUpdateMap());
  }

  Future<void> removeById(int id, {Connection? connection}) async {
    final conn = connection ?? db;
    await conn.table(FaixaTempoResidencia.fqtn).where('id', '=', id).delete();
  }

  Future<void> removeAllInTransaction(List<FaixaTempoResidencia> items) async {
