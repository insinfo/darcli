...
@Component(
  selector: 'form-tipo-conhecimento-page',
  templateUrl: 'form_tipo_conhecimento_page.html',
  styleUrls: ['form_tipo_conhecimento_page.css'],
  directives: [
    coreDirectives,
...
class FormTipoConhecimentoPage implements OnActivate {
  ...
  FormTipoConhecimentoPage(this.notificationComponentService, this.hostElement,
      this._tipoConhecimentoService, this._router, this._location);

  var item = TipoConhecimento(nome: '', id: -1);
  ...
  Future<void> getById(int id) async {
    ...
    try {
      ...
      item = await _tipoConhecimentoService.getById(id);
    } catch (e, s) {
      print('FormTipoConhecimentoPage@getById $e $s');
  ...
  Future<void> salvar(NgForm cadastroForm, {bool showLoading = true}) async {
    ...
    if (isValid == false) {
      ...
      try {
        ...
        if (isNew) {
          await _tipoConhecimentoService.insert(item);
        } else {
          await _tipoConhecimentoService.update(item);
        }
        notificationComponentService.notify(
            'Tipo Conhecimento ${isNew ? 'cadastrado' : 'atualizado'} com sucesso.');

        _router.navigate(RoutePaths.listaTipoConhecimento.toUrl());
      } catch (e, s) {
        print('FormTipoConhecimentoPage@load $e $s');
        SimpleDialogComponent.showAlert(StatusMessage.ERROR_GENERIC,
            subMessage: '$e $s');
      } finally {
        if (showLoading) {
