...
import 'package:darcli/darcli.dart' as darcli;
import 'package:darcli/src/extensions/extensions.dart';
...
void main() {
  ...
  setUp(() async {
    dir = await Directory.systemTemp.createTemp('darcli.test.');
  });
  ...
  test('Validate pkg/darcli pubspec', () {
  ...
  group('generator', () {
    for (var generator in darcli.generators) {
      test(generator.id, () {
...
void _testGenerator(darcli.Generator generator, Directory tempDir) {
  Dart.run(path.join(path.current, 'bin/darcli.dart'),
  ...
  if (path.extension(filePath) != '.dart' ||
    ...
    final file = _listSync(parent)
        .firstWhereNull((f) => f.path.endsWith('.dart'));
  ...
  expect(pubspecContent, containsPair('name', 'darcli'));
